...
def create_quick_task(summary):
    """Create a Task directly in the AX project backlog with the PM template.
    No AI enrichment — just creates the ticket immediately.
    Returns (task_key, task_url) or (None, None) on failure."""

    # If summary is too long for Jira title (255 char limit), truncate
    jira_title = summary[:250].rsplit(' ', 1)[0] + "..." if len(summary) > 250 else summary
